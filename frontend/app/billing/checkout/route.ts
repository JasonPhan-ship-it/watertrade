  ...
  try {
    ...
    if (process.env.STRIPE_SECRET_KEY && process.env.STRIPE_PRICE_ID) {
      try {
        ...
        // Save Stripe customer ID if returned (with error handling for missing fields)
        if (session.customer) {
          try {
            ...
            // If stripeCustomerId field doesn't exist, just log and continue
            if (prismaError.message?.includes("Unknown field")) {
              console.log("stripeCustomerId field not found in schema, skipping save");
            } else {
              throw prismaError;
            }
          }
    ...
    await prisma.user
      .update({
        where: { id: user.id },
        data: {
          subscriptionStatus: "contact_required",
          subscriptionUpdatedAt: new Date(),
        },
      })
      .catch(() => {});
