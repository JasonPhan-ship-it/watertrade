...
async function fetchVercelSeries(
  start: Date,
  end: Date
): Promise<AdminOverviewTimeseriesPoint[] | null> {
...
function parseVercelPayload(payload: any): AdminOverviewTimeseriesPoint[] | null {
  ...
  for (const series of seriesCandidates) {
    const normalized = series
      ...
      .filter(Boolean);
    if (normalized.length) return normalized as AdminOverviewTimeseriesPoint[];
  }
...
async function buildSignupSeries(
...
): Promise<AdminOverviewTimeseriesPoint[]> {
  ...
  const series: AdminOverviewTimeseriesPoint[] = [];
...
export async function GET() {
  ...
  const payload: AdminOverviewMetrics = {
    revenue: {
