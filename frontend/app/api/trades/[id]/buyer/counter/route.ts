...
// If you have a seller-facing template, import it. If not, we'll fallback.
// import { renderSellerCounterEmail } from "@/lib/email";
...
async function renderSellerCounterEmailSafe(args: any): Promise<RenderOut> {
  ...
  } catch {/* no template, fallback below */}
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // Server-side guard: price cannot be lower than current offer
    ...
    // Update Trade as COUNTERED_BY_BUYER
    ...
    // Enrich with Clerk if available
    ...
    if (sellerEmail) {
      ...
      const declineLink = appUrl(`/t/${trade.id}?role=seller&token=${trade.sellerToken}&action=decline`);
    ...
    // Return JSON for client modal success
