...
// Match renderer shape without changing lib/email right now
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    // 1) Resolve viewer + trade from either Trade.id OR Transaction.id (optionally creates Trade)
    ...
    if (pricePerAfNum <= 0 || volumeAfNum <= 0) {
      return NextResponse.json(
        { error: "pricePerAf and volumeAf must be greater than 0", errorCode: "VALIDATION_ERROR" },
        { status: 400 }
      );
    }
    ...
    if (typeof trade.pricePerAf === "number" && pricePerAfNum > trade.pricePerAf) {
      return NextResponse.json(
        { error: `Buyer counter should be at most ${(trade.pricePerAf / 100).toFixed(2)} USD/AF.`, errorCode: "BUSINESS_RULE" },
        { status: 400 }
      );
    }
    ...
    const sellerViewLink = appUrl(`/t/${updated.id}?role=seller${trade.sellerToken ? `&token=${trade.sellerToken}` : ""}`);
    const sellerCounterLink = `${sellerViewLink}&action=counter`;
    const sellerDeclineLink = `${sellerViewLink}&action=decline`;

    // Email seller about buyer counter
    if (sellerEmail) {
      const { html, preheader } = renderBuyerCounterEmailSafe({
        ...
        counterLink: sellerCounterLink,
        declineLink: sellerDeclineLink,
      });

      await sendEmail({ to: sellerEmail, subject: "Buyer sent a counteroffer", html, preheader });
    }

    // NEW: Email buyer a confirmation copy
    if (buyerEmail) {
      const buyerViewLink = appUrl(`/t/${updated.id}?role=buyer${updated.buyerToken ? `&token=${updated.buyerToken}` : ""}`);
      const html = `
        <p>Hi ${buyerName || "Buyer"},</p>
        <p>Your counteroffer was sent to the seller.</p>
        <ul>
        ...
        <p>You can view the thread here: <a href="${buyerViewLink}">${buyerViewLink}</a></p>
      `;
      await sendEmail({ to: buyerEmail, subject: "Your counteroffer was sent", html });
    }
