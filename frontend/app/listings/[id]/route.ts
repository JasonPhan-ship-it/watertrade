...
export async function GET(req: Request, ctx: { params: { id: string } }) {
  try {
    const id = decodeURIComponent(ctx.params.id); // ← decode once

    // Build an absolute URL to call your existing list endpoint.
    const url = new URL(req.url);
    const base = `${url.protocol}//${url.host}`;

    // Pull a reasonably large page so the id is included.
