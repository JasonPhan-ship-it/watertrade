...
const WESTLANDS_DISTRICT = "Westlands Water District";
...
type StepKey = "district" | "westlands" | "profile";
...
export default function OnboardingPage() {
  ...
  const [step, setStep] = React.useState<StepKey>("district");
  const [isInWestlands, setIsInWestlands] = React.useState<boolean | null>(null);
  ...
  React.useEffect(() => {
    if (isInWestlands !== null) return;
    if (loadingWestlands) return;

    if (westlandsIntegration?.status === "CONNECTED" || westlandsIntegration?.status === "DECLINED") {
      setIsInWestlands(true);
      setStep("westlands");
      setPresetSelected((prev) => {
        if (prev.has(WESTLANDS_DISTRICT)) return prev;
        const next = new Set(prev);
        next.add(WESTLANDS_DISTRICT);
        return next;
      });
    }
  }, [isInWestlands, loadingWestlands, westlandsIntegration?.status]);
  ...
  const handleDistrictAnswer = (answer: boolean) => {
    setIsInWestlands(answer);
    setPresetSelected((prev) => {
      const next = new Set(prev);
      if (answer) {
        next.add(WESTLANDS_DISTRICT);
      } else {
        next.delete(WESTLANDS_DISTRICT);
      }
      return next;
    });
    if (!answer) {
    ...
    setStep(answer ? "westlands" : "profile");
    scrollToTop();
  };

  const goToDistrictStep = () => {
    setStep("district");
    scrollToTop();
  };

  const goToWestlandsStep = () => {
    setStep("westlands");
    scrollToTop();
  };
  ...
  return (
    <div className="mx-auto max-w-2xl p-6">
              ...
              <li key={item.key} className="flex items-center gap-3">
                <span
                  className={`flex h-8 w-8 items-center justify-center rounded-full border text-sm font-semibold ${
                    isActive
                      ? "border-[#004434] bg-[#004434] text-white"
                      : isComplete
      ...
      {debugInfo.length > 0 && (
        <details className="mt-6 rounded-lg bg-yellow-50 p-3 text-sm">
          <summary className="cursor-pointer font-medium text-yellow-800">Debug Information (click to expand)</summary>
      ...
      {step === "district" && (
        <section className="mt-8 space-y-6 rounded-2xl border border-slate-200 bg-white/70 p-6 shadow-sm">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">Welcome to Water Traders</h1>
            <p className="mt-2 text-slate-600">Let&apos;s start by confirming which water district you&apos;re in.</p>
          </div>
          <div className="space-y-5">
            <p className="text-sm text-slate-700">Are you a grower in the Westlands Water District?</p>
            <div className="grid gap-3 sm:grid-cols-2">
              <button
                type="button"
                onClick={() => handleDistrictAnswer(true)}
                className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-800 shadow-sm transition hover:bg-emerald-100"
              >
                Yes, I&apos;m in Westlands
              </button>
              <button
                type="button"
                onClick={() => handleDistrictAnswer(false)}
              ...
              >
                No, I&apos;m in a different district
              </button>
            </div>
            <p className="text-xs text-slate-500">You can list every district you work with in the next step.</p>
          </div>
        </section>
      )}

      {step === "westlands" && isInWestlands !== false && (
        <section className="mt-8 space-y-6 rounded-2xl border border-slate-200 bg-white/60 p-6 shadow-sm">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <button
              type="button"
              onClick={goToDistrictStep}
              className="inline-flex items-center gap-2 self-start rounded-lg border border-slate-300 px-3 py-1.5 text-sm text-slate-700 transition hover:bg-slate-50"
            >
              ← Back
            </button>
          </div>
                ...
                <div className="space-y-4">
                  <div>
                    <p className="text-sm text-slate-700">
                      Would you like Water Traders to access your Westlands Water District account so we can display your live water balance?
                    </p>
                    {westlandsStatus === "DECLINED" && (
                      <p className="mt-1 text-xs text-slate-500">
                        You can change your preference at any time—select “Yes” whenever you&apos;re ready.
                      </p>
                    )}
          ...
          <div className="flex flex-col-reverse gap-3 pt-4 sm:flex-row sm:items-center sm:justify-between">
            <button
              type="button"
              onClick={goToDistrictStep}
              className="inline-flex items-center gap-2 rounded-lg border border-slate-300 px-3 py-1.5 text-sm text-slate-700 transition hover:bg-slate-50"
            >
              ← Back
            </button>
            <button
              ...
              disabled={!canContinueFromWestlands}
              className="inline-flex items-center justify-center rounded-lg bg-[#004434] px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-[#00392f] disabled:opacity-40"
            >
              Continue to profile
            </button>
          </div>
 ...
 {step === "profile" && (
        <section className="mt-8 space-y-6">
          ...
          <form onSubmit={onSubmit} className="space-y-6 rounded-2xl border border-slate-200 bg-white/70 p-6 shadow-sm">
            {/* Name */}
 ...
 <span>{d}</span>
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full rounded-xl bg-[#004434] px-5 py-2 text-white hover:bg-[#003a2f] disabled:opacity-50"
            >
