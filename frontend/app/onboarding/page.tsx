...
export default function OnboardingPage() {
  ...
  React.useEffect(() => {
    ...
    // Not signed in -> send to sign-in and return here
    if (!isSignedIn) {
    ...
    // If Clerk metadata already says onboarded and not forced, skip
    if (!force && user?.publicMetadata?.onboarded === true) {
      goNext();
      return;
    }

    // Otherwise ask our onboarding API
    let active = true;
    ...
    (async () => {
      try {
        ...
        if (!force && onboarded) {
          // Optional: set a client cookie for quick future checks
          if (user?.id) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      // Write via onboarding init (marks onboarded = true)
      const res = await fetch("/api/onboarding/init", {
        method: "POST",
        credentials: "include",
      ...
      // Optional client cookie (non-HttpOnly) for faster checks
      if (user?.id) {
        document.cookie = `onboarded=${encodeURIComponent(String(user.id))}; Path=/; Max-Age=1800; SameSite=Lax`;
      }

      // If you want to skip the membership step, replace the next line with: goNext();
