// frontend/app/homepage-client.tsx
"use client";
...
function useHomepageSettings(initialCopy: HomepageCopy, initialFeeRate: number) {
  const [copy, setCopy] = React.useState<HomepageCopy>(initialCopy);
  const [feeRate, setFeeRate] = React.useState<number>(initialFeeRate);
    ...
    async function load() {
      try {
        ...
        if (!cancelled && copyRes.ok) {
          ...
          if (json?.value) {
            setCopy(mergeHomepageCopy(initialCopy, json.value ?? {}));
          }
        }
...
export default function HomepageClient({ initialCopy, initialFeeRate }: HomepageClientProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const { isSignedIn } = useUser();
  const { data, loading, error } = useListingsPreview();
  const { copy, feeRate } = useHomepageSettings(initialCopy, initialFeeRate);
